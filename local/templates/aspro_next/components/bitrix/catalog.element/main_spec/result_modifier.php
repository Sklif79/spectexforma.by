...
<?
$arResult["SERTFICATIONS"] = array();
if(isset($arResult["PROPERTIES"]["SERTFICATIONS"]["VALUE"]) && is_array($arResult["PROPERTIES"]["SERTFICATIONS"]["VALUE"]))
{
    foreach($arResult["PROPERTIES"]["SERTFICATIONS"]["VALUE"] as $FILE)
    {
        $FILE = CFile::GetFileArray($FILE);
        if(is_array($FILE))
            $arResult["SERTFICATIONS"][]=$FILE;
    }
}
?>
<?
//$arSelect = Array("ID","IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PROPERTY_LINK_ELEM_COLOR",);
//$arFilter = Array("ID" => $arResult["ID"],"IBLOCK_ID"=>18,"PROPERTY_LINK_ELEM_COLOR" );
//$res = CIBlockElement::GetList(Array(), $arFilter, false, false);
//while($ob = $res->GetNextElement())
//{
//$arFieldsNews = $ob->GetFields();
//$arFieldsNewsProps = $ob->GetProperties();
// debug($arFieldsNewsProps);
//}
...
<?
/*Собираем и выводим в массив $arResult['LINK_CAT_ELEM'] элементы со свойством LINK_ELEM_COLOR карточки */
$arItemId = [];
foreach($arResult["PROPERTIES"]["LINK_ELEM_COLOR"]["VALUE"] as $analog){
    $arItemId[] = $analog;

};
/*Получаем */
$arSelect = Array("ID","IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PROPERTY_LINK_ELEM_COLOR",'PROPERTY_COLOR_REF2');
$arFilter = Array("ID" => $arItemId,"IBLOCK_ID"=>18,"PROPERTY_LINK_ELEM_COLOR", 'PROPERTY_COLOR_REF2', 'ACTIVE' => 'Y');
$res = CIBlockElement::GetList(Array(), $arFilter, false, false, $arSelect);
while($ob = $res->GetNextElement())
{
    $arFields = $ob->GetFields();
    $props = $ob->GetProperties();
    $arResult['CAT_ELEM'][$arFields["ID"]] = $arFields;
}
foreach ($arResult["CAT_ELEM"] as $value) {
    $color_codes[] = $value['~PROPERTY_COLOR_REF2_VALUE'];
...
$arHLBlock = Bitrix\Highloadblock\HighloadBlockTable::getById(3)->fetch(); // 1 - ID HL(!) инфоблока
...
$arLink = [];
$i=0;
foreach ($arResult["CAT_ELEM"] as $key => $value) {
    $getFile = CFile::GetPath($ar_res[$value['PROPERTY_COLOR_REF2_VALUE']]);
    $arLink[$i]['src'] = $getFile;
    $arLink[$i]['link'] = $value['DETAIL_PAGE_URL'];
    $i++;
}
$arResult['LINK_CAT_ELEM'] = $arLink;
...
/*Аналогичный метод */
/*$arSelect = Array("ID","IBLOCK_ID", "NAME", "DETAIL_PAGE_URL", "PREVIEW_PICTURE", "PROPERTY_LINK_ELEM_COLOR",'PROPERTY_COLOR_REF2');
$arFilter = Array("ID" => $arItemId,"IBLOCK_ID"=>18,"PROPERTY_LINK_ELEM_COLOR", 'PROPERTY_COLOR_REF2' );
