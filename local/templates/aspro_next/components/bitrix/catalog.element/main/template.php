...
<?foreach ($arResult["OFFERS"] as $key => $arOffer){
	//debug($arOffer['PROPERTIES']['VOLUME']['VALUE']);
}?>
<?//debug($arBasketPrices)?>
...
<div class="item_main_info <?=(!$showCustomOffer ? "noffer" : "");?> <?=($arParams["SHOW_UNABLE_SKU_PROPS"] != "N" ? "show_un_props" : "unshow_un_props");?>" id="<?=$arItemIDs["strMainID"];?>">
			...
			<?if(!$showCustomOffer || empty($arResult['OFFERS_PROP'])){
				if(count($arResult["MORE_PHOTO"]) > 1):?>
					<div class="wrapp_thumbs xzoom-thumbs">
						<div class="thumbs flexslider" data-plugin-options='{"animation": "slide", "selector": ".slides_block > li", "directionNav": true, "itemMargin":10, "itemWidth": 54, "controlsContainer": ".thumbs_navigation", "controlNav" :false, "animationLoop": true, "slideshow": false}' style="max-width:<?=ceil(((count($arResult['MORE_PHOTO']) <= 4 ? count($arResult['MORE_PHOTO']) : 4) * 72) - 10)?>px;">
							<ul class="slides_block" id="thumbs">
								<?foreach($arResult["MORE_PHOTO"]as $i => $arImage):?>
									<li <?=(!$i ? 'class="current"' : '')?> data-big_img="<?=$arImage["BIG"]["src"]?>" data-small_img="<?=$arImage["SMALL"]["src"]?>">
										<span>
                                            <img class="xzoom-gallery"
                                                 width="70" xpreview="<?=$arImage["THUMB"]["src"];?>"
	...
	<div  class="right_info">
		<div class="info_item">
				...
				<div class="top_info">
					<div class="rows_block">
						...
						<?if($arResult["BRAND_ITEM"]){?>
							<div class="float-right item_block col-<?=$col;?>">
								<div class="brand">
					...
					<span class="block_title-art" itemprop="name">Артикул: </span>
					<span class="value-art" itemprop="value"><?=$arResult['PROPERTIES']['CML2_ARTICLE']['VALUE']?></span>
            ...
            <?if(!empty($arResult['PROPERTIES']['LINK_ELEM_COLOR']['VALUE'])){?>
                <div class="sku_props_color">
                    <span>
                        Цвет
                    </span>
                    <ul>
                        <?if(!empty($arResult['PROPERTIES']['LINK_ELEM_COLOR']['VALUE'])){?>
                            <?
                            foreach ($arResult['LINK_CAT_ELEM'] as $value) {?>
                                <li>
                                    <a href="<?=$value['link']?>" alt="<?=$value['link']?>">
                                        <img src="<?=$value['src']?>"
                                             alt="<?=$value['link']?>"
		...
		<?endif;*/?>
        <div class="color-txt">При выборе цвета вы переходите на страницу товара другого цвета</div>
        ...
        <div class="discount-prod"
            ...
            <?}?>
        >
            <div class="discount-prod-img">
                <img src="/upload/product-images/discount.png" alt="">
            </div>
            <div class="discount-prod-txt">
                <p>Скидки действуют <a class="discount-types" href="#discounts">на общую сумму заказа</a> любых товаров</p>
            </div>
        </div>
        <div class="first-characteristics">
			<ul class="first-characteristics--inner">
				<?$kull = 0;?>
				...
				?>
					<?if(!in_array($arProp["CODE"], array("SERVICES", "BRAND", "HIT", "RECOMMEND", "NEW", "STOCK", "VIDEO", "VIDEO_YOUTUBE", "CML2_ARTICLE", 'LINK_ELEM_COLOR'))):?>
				...
				<?$kull=$kull+1; endforeach;?>
			</ul>
            <!--<ul class="first-characteristics--inner">
                <li><span>Материал:</span> Шэлтон</li>
            </ul>-->
        </div>
        <div class="more-character">
            <a href="#character" class='characterMy' >Подробнее <img src="/upload/product-images/angle-arrow-down.png" alt=""></a>
        </div>
		</div>
	</div>
        <div class="table-tablet">
            <div class="table-size--inner">
                <div class="table-s-title">
                    Выберите размер и количество
                </div>
                <div class="table-s-title-pop">
                    <img src="/upload/product-images/icon-tbl.png" alt="">
                    <a class="popup-table-size" href="#size-popup"> Таблица размеров</a>
                </div>

                <div class="table-tablet-main">
					<!--<ul class="table-size--content">
						<li class="table-size--content-item">
							<p class="table-size--content-title">44-46/158-164</p>
							<div class="table-size--content-count">
								<button type="button" class="table-count count-down"></button>
								<input type="text" value="100">
								<button type="button" class="table-count count-up"></button>
							</div>
						</li>
						<li class="table-size--content-item">
							<p class="table-size--content-title">44-46/158-164</p>
						...
						<li class="table-size--content-item">
							<ul class="price-large-sizes">
								<li>
									<a href="#" class="table-size--content-title-link">64-66/194-200</a>
									<ul>
										<li>
											<span>120 000 руб.</span>
											<p>Цена для больших размеров</p>
										</li>
									</ul>
								</li>
							</ul>
						...
						<li class="table-size--content-item">
							<p class="table-size--content-title">S</p>
							<div class="table-size--content-count">
								<button type="button" class="table-count count-down"></button>
								<input type="text" value="100">
								<button type="button" class="table-count count-up"></button>
						...
						<li class="table-size--content-item">
							<p class="table-size--content-title">M</p>
							<div class="table-size--content-count">
								<button type="button" class="table-count count-down"></button>
								<input type="text" value="100">
								<button type="button" class="table-count count-up"></button>
						...
						<li class="table-size--content-item">
							<p class="table-size--content-title">Количество</p>
							<div class="table-size--content-count">
								<button type="button" class="table-count count-down"></button>
								<input type="text" value="100">
								<button type="button" class="table-count count-up"></button>
						...
						<li class="table-size--content-item">
							<p class="table-size--content-title">36</p>
							<div class="table-size--content-count">
								<button type="button" class="table-count count-down"></button>
								<input type="text" value="100">
								<button type="button" class="table-count count-up"></button>
						...
						<li class="item-total first-item-total">
							<p>Сумма заказа:</p>
							<p>100 000 руб.</p>
						</li>
						<li class="item-total">
                    ...
                    <ul class="table-size--content">
                        <?if($arResult["OFFERS"]){?>
                            <?$arItemProps = (($arParams["OFFERS_CART_PROPERTIES"]) ? implode(';', $arParams["OFFERS_CART_PROPERTIES"]) : "");?>
                            ...
                            >
                                ...
                                <?foreach ($arResult["OFFERS"] as $key => $arOffer){
                                    if($arOffer['CAN_BUY']){?>
                                        <?
                                        $priceOffer = $arOffer['PRICES']['BASE']['VALUE'];
                                        ?>
                                        <li class="table-size--content-item" >
                                            <?//TODO: вместо тега p тут по весртке ul при условии что в торговом предложении у свойства SPEC_PRICE стоит Y ?>

                                            <?if(!empty($arOffer['PROPERTIES']['SPEC_PRICE']) && $arOffer['PROPERTIES']['SPEC_PRICE']['VALUE'] == 'Y'){?>
                                                <ul class="price-large-sizes">
                                                    <li>
                                                        <a href="javascript:void(0)" class="table-size--content-title-link"><?echo $arOffer['NAME'];?></a>
                                                        <ul>
                                                            <li>
                                                                <span><?=$arOffer['PRICES']['BASE']['PRINT_DISCOUNT_VALUE_NOVAT']?></span>
                                                                <p>Цена для больших размеров</p>
                                                            </li>
                                                        </ul>
                                                    </li>
                                            ...
                                            <?} else {?>
                                                <p class="table-size--content-title">
                                                    <?echo $arOffer['NAME'];?>
                                                </p>
                                            <?}?>

                                            <div class="table-size--content-count">
                                                <a class="MinusList table-count count-down" href="javascript:void(0)"
                                                   data-key='<?=$key?>'
                                                   onclick="BX('<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>_<?=$arOffer['ID']?>').value--;">
                                                </a>
                                                <input type="hidden" name="price" value="<?=$priceOffer?>>" class='getPriceValue'>
                                                <input type="text"
                                                       class='getInputValue'
                                                       name="<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>_<?=$arOffer['ID']?>"
                                                       placeholder='0'
                                                       value='0'
                                                       min='0'
                                                       id="<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>_<?=$arOffer['ID']?>">
                                                <a class="PlusList table-count count-up" href="javascript:void(0)"
                                                   data-key='<?=$key?>'
                                                   onclick="BX('<?echo $arParams["PRODUCT_QUANTITY_VARIABLE"]?>_<?=$arOffer['ID']?>').value++;">
                                                </a>
                                            </div>
                                        ...
                                        <input type="hidden" name="key_parent" value="<?echo $arOffer['PROPERTIES']['CML2_LINK']['VALUE']?>">
                                        <input type="hidden" name="iblockID" value="<?=$arResult['ORIGINAL_PARAMETERS']['IBLOCK_ID']?>">
                                        <input type="hidden" name="props" value="<?=$arItemProps?>">
                                        <input type="hidden" name="own_id_<?=$arOffer["ID"]?>[]" value="own_id_<?=$arOffer["ID"]?>">
                            ...
                            <div class="table-tablet-bottom">
                                <ul class="table-size--content tablet-total">
                                    <!--<li class="item-total first-item-total">
                                        <p>Сумма заказа:</p>
                                        <p>100 000 руб.</p>
                                    </li>-->
                                    <li class="item-total first-item-total item-total-sum">
                                        <p>Сумма заказа:</p>
                                        <p></p>
                                    </li>
                                    ...
                                    <li class="item-total discountPrice">
                                        <p>Ваша скидка:</p>
                                        <p></p>
                                    </li>
                                    ...
                                    <li class="item-total totalPrice">
                                        <p><strong>Итого:</strong></p>
                                        <p><strong></strong></p>
                                    </li>
                                ...
                                <div class="table-buttons tablet-table-buttons">
                                    <a href='javascript:void(0)'
                                       class='addtoCardbyAnkuse in-basket'>В корзину</a>
                                    <span class="btn btn-default white btn-lg type_block transition_bg one_click buy-fast" data-item="<?=$arResult["ID"]?>" data-iblockID="<?=$arParams["IBLOCK_ID"]?>" data-quantity="<?=$arAddToBasketData["MIN_QUANTITY_BUY"];?>" onclick="oneClickBuy('<?=$arResult["ID"]?>', '<?=$arParams["IBLOCK_ID"]?>', this)">
                                    <span><?=GetMessage('ONE_CLICK_BUY')?></span>
                                </span>
                                </div>
                                ...
                                <!--<div class="table-buttons tablet-table-buttons">
                                    ...
                                    <a class="buy-fast" href="#">Купить в 1 клик</a>
                                </div>-->
                            </div>
                            ...
                            <script>
                                $( document ).ready(function() {
                                    /*Ставим нулевые значения при загрузке*/
                                    $('.item-total-sum>p:nth-child(2n)').text('0 руб.');
                                    $('.discountPrice>p:nth-child(2n)').text('0 руб.');
                                    $('.totalPrice>p:nth-child(2n)>strong').text('0 руб.');

                                    function getSum(index, change) {
                                        arPrice.splice(index, 1, change);
                                        console.log(arPrice);
                                    }
                                    ...
                                    $('.PlusList, .MinusList, .getInputValue').on('click keyup',function () {
                                        var getVal = parseInt($(this).parent().find('.getInputValue').val());
                                        var getPrice = parseInt($(this).parent().find('.getPriceValue').val());
                                        var sum = getVal * getPrice;
                                        ...
                                        var strSum = number_format(sum_sku, 0, '', ' ');
                                        $('.item-total-sum>p:nth-child(2n)').text(sum_sku + ' руб.');
                                        Number(sum_sku);
                                        console.log('numberTypeSumSku', typeof(sum_sku));
                                        //console.log(typeof(sum_sku));
                                        var lastRange = 0;
                                        var lastDiscount = 0;
                                        <?foreach ($arResult['PROPERTIES']['SKIDKI']['VALUE'] as $key => $skidki){
                                        $diapazon = (explode( ',', $skidki, 2));
                                        $ot= $diapazon[0];
                                        $do= $diapazon[1];
                                        $discValue = [];
                                        foreach ($arResult['PROPERTIES']['SKIDKI']['DESCRIPTION'] as $keySkidki => $discountval){
                                            $discVal = array_push($discValue,$discountval);
                                        }?>
                                        var per_ot_<?=$key?> = <?=$ot?>;
                                        var per_do_<?=$key?> = <?=$do?>;
                                        var per_discount = <?=$discValue[$key]?>;
                                        if(sum_sku >= per_ot_<?=$key?> && sum_sku <= per_do_<?=$key?>) {
                                            /*расчет и установка поля скидка*/
                                            var discountPercent = per_discount; // процент скидки от общего кол-ва (брать из админки?)
                                            var skidka = (sum_sku*discountPercent)/100;
                                            $('.discountPrice>p:nth-child(2n)').text(skidka+ ' руб');

                                            /*расчет и установка поля итого*/
                                            var totalPrice = sum_sku-skidka;
                                            var total = number_format(totalPrice, 0, '', ' ');
                                            $('.totalPrice>p:nth-child(2n)>strong').text(total+ ' руб.');
                                            Number(totalPrice);
                                        }
                                        lastRange = <?=$do?>;
                                        lastDiscount = <?=$discValue[$key]?>;
                                        <?if($key == end(array_keys($arResult['PROPERTIES']['SKIDKI']['VALUE']))){
                                        break;
                                    }?>
                                        <?}?>
                                        if(sum_sku >= lastRange) {
                                            var skidka = (sum_sku*lastDiscount)/100;
                                            var formatSkidka = number_format(skidka, 0, '', ' ');
                                            $('.discountPrice>p:nth-child(2n)').text(formatSkidka+ ' руб.');
                                            Number(formatSkidka);

                                            var totalPrice = sum_sku-skidka;
                                            var totalFormated = number_format(totalPrice, 0, '', ' ');
                                            $('.totalPrice>p:nth-child(2n)>strong').text(totalFormated+ ' руб.');
                                            Number(totalFormated);
                                        }
                                    });
                                });
                            </script>
        ...
        <div class="table-mobile">
            <div class="table-size--inner mobileMy" id="mobileMy">
                <div class="table-s-title">
                    Выберите размер и количество
                </div>
                <div class="table-s-title-pop">
                ...
                <ul class="table-size--content">
                    <?if($arResult["OFFERS"]){?>
                        <?$arItemProps = (($arParams["OFFERS_CART_PROPERTIES"]) ? implode(';', $arParams["OFFERS_CART_PROPERTIES"]) : "");?>
                        <form action="<?=POST_FORM_ACTION_URI?>" method="POST" enctype="multipart/form-data" class='aff_form'>
                            <script>
                                var arPrice =[]
                            </script>
                            <?foreach ($arResult["OFFERS"] as $key => $arOffer){
                        ...
                        <script>
                            $( document ).ready(function() {
                                ...
                                $('.PlusList, .MinusList, .getInputValue').on('click keyup',function () {
                                    ...
                                    var strSum = number_format(sum_sku, 0, '', ' ');
                                    $('.item-total-sum>p:nth-child(2n)').text(strSum + ' руб.');
                                    Number(sum_sku);
                                    console.log('numberTypeSumSku', typeof(sum_sku));
                                    //console.log(typeof(sum_sku));
                                    var lastRange = 0;
...
<a name="character" style="opacity:0; position: relative;top:-96px;">&nbsp;</a>
<div class="tabs_section">
	...
	<div class="tabs" id="character">
		...
		<ul  class="nav nav-tabs">
			...
			<?if($arResult["DETAIL_TEXT"] || $arResult['ADDITIONAL_GALLERY'] || count($arResult["SERVICES"]) || ((count($arResult["PROPERTIES"][$instr_prop]["VALUE"]) && is_array($arResult["PROPERTIES"][$instr_prop]["VALUE"])) || count($arResult["SECTION_FULL"]["UF_FILES"])) || ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):?>
				<li  class="descr_anchor <?=(!($iTab++) ? ' active' : '')?>">
					<a href="#descr" data-toggle="tab"><span><?=($arParams["TAB_DESCR_NAME"] ? $arParams["TAB_DESCR_NAME"] : GetMessage("DESCRIPTION_TAB"));?></span></a>
				</li>
			<?endif;?>
            <?if($arParams["PROPERTY_GOODS"] == "Y"):?>
                <li class="anchor_props <?=(!($iTab++) ? ' active' : '')?>">
                    <a href="#props_goods" data-toggle="tab">
                        <span>
                            <?=GetMessage("PROPERTY_GOODS");?>
                        </span>
                    </a>
                </li>
            <?endif;?>
            <?if($arParams["PAY_AND_DELIVERY"] == "Y"):?>
                <li class="<?=(!($iTab++) ? ' active' : '')?>">
                    <a href="#pay" data-toggle="tab">
                        <span>
                            <?=GetMessage("PAY_AND_DELIVERY");?>
                        </span>
                    </a>
                </li>
            <?endif;?>
            <?if($arParams["SERTIFICATIONS"] == "Y"):?>
                <li class="<?=(!($iTab++) ? ' active' : '')?>">
                    <a href="#sertfications" data-toggle="tab">
                        <span>
                            <?=GetMessage("SERTFICATIONS");?>
                        </span>
                    </a>
                </li>
            <?endif;?>
		...
		<div  class="tab-content">
			...
			<?if($arResult["DETAIL_TEXT"] || count($arResult["SERVICES"]) || ((count($arResult["PROPERTIES"][$instr_prop]["VALUE"]) && is_array($arResult["PROPERTIES"][$instr_prop]["VALUE"])) || $arResult['ADDITIONAL_GALLERY'] || count($arResult["SECTION_FULL"]["UF_FILES"])) || ($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB")):?>
				<div class="descr tab-pane <?=(!($iTab++) ? ' active' : '')?>" id="descr">
                    ...
                    <div class="description--full">
                        ...
                        <div class="just-text">
                            ...
                            <?/* в script.js есть закоменченный скрипт который надо раскомментить чтобы заработал код "подробнее"! */?>
                            <?/*if(strlen($arResult["DETAIL_TEXT"]) > 535 ){*/?><!--
			...
			<?endif;?>
            <?if($arParams["PAY_AND_DELIVERY"] == "Y"):?>
                <div class="tab-pane pay_block<?=(!($iTab++) ? ' active' : '')?>" id="pay">
                    <div class="title-tab-heading visible-xs">
                        <?=GetMessage("PAY_AND_DELIVERY");?>
                    </div>
                    <div>
                        <?// TODO: Тут берется из отдельного файла, но можно сделать из инфоблока,
                          // TODO: так удобнее и не надо лезть в код. ?>
                        <?/*$APPLICATION->IncludeFile(SITE_DIR."include/additional_products_description.php",
                            array(),
                            array("MODE" => "html",
                                "NAME" => GetMessage('CT_BCE_CATALOG_ADDITIONAL_DESCRIPTION')));
                        */?>
                        <?// TODO: Поле в инфоблоке PAY_AND_DELIVERY?>
                        <?// TODO: В настройках компонента выставить свойсто активным?>
                        <?=$arResult['PROPERTIES']['PAY_AND_DELIVERY']['VALUE']['TEXT']?>
                    </div>
                </div>
            <?endif;?>
            <?if($arParams["PROPERTY_GOODS"] == "Y"):?>
                <div class="props_goods tab-pane pay_block<?=(!($iTab++) ? ' active' : '')?>" id="props_goods">
                    <div class="title-tab-heading visible-xs">
                        <?=GetMessage("PROPERTY_GOODS");?>
                    </div>
                    <?if($showProps && $arParams["PROPERTIES_DISPLAY_LOCATION"] != "TAB"):?>
                        <div class="wraps">
                            ...
                            <?if($arParams["PROPERTIES_DISPLAY_TYPE"] != "TABLE"): // Блочная верстка?>
                                <div class="props_block" id="<? echo $arItemIDs["ALL_ITEM_IDS"]['DISPLAY_PROP_DIV']; ?>">
                                    <?foreach($arResult["PROPERTIES"] as $propCode => $arProp):?>
                                        <?if(isset($arResult["DISPLAY_PROPERTIES"][$propCode])):?>
                                            <?$arProp = $arResult["DISPLAY_PROPERTIES"][$propCode];?>
            ...
            <?if($arParams["SERTIFICATIONS"] == "Y"):?>
                <div class="tab-pane sert_block<?=(!($iTab++) ? ' active' : '')?>" id="sertfications">
                    <div class="title-tab-heading visible-xs">
                        <?=GetMessage("SERTFICATIONS");?>
                    </div>
    ...
    justify-content: space-around;">
                        <?foreach($arResult["SERTFICATIONS"] as $photo){?>
                            <?if ($arResult["SERTFICATIONS"]) {?>
                                <?$sertFoto = CFile::ResizeImageGet($photo['ID'], Array("width" => 200, "height" => 200));?>
                                <a href="<?=$photo['SRC']?>"
...
<?/*вверху есть блок который открывает , искать по поиску
if($arParams["WIDE_BLOCK"] == "Y"):*/?><!--
		</div>
		<div class="col-md-3">
                    ...
                    <form action="<?=POST_FORM_ACTION_URI?>" method="POST" enctype="multipart/form-data" class='aff_form'>
                        ...
                        <?foreach ($arResult["OFFERS"] as $key => $arOffer){
                            ...
                            if($arOffer['CAN_BUY']){?>
                                ...
                                <input type="hidden" name="props" value="<?=$arItemProps?>">
                                <input type="hidden" name="own_id_<?=$arOffer["ID"]?>[]" value="own_id_<?=$arOffer["ID"]?>">
                                <input type="hidden" name="volume_<?=$arOffer["ID"]?>" value="<?=$arOffer['PROPERTIES']['VOLUME']['VALUE']?>">
                        ...
                        $( document ).ready(function() {
                            ...
                            $('.PlusList, .MinusList, .getInputValue').on('click keyup',function () {
                                ...
                                <?foreach ($arResult['PROPERTIES']['SKIDKI']['VALUE'] as $key => $skidki){
                                    ...
                                    var per_discount = <?=$discValue[$key]?>;
                                    if(sum_sku >= per_ot_<?=$key?> && sum_sku <= per_do_<?=$key?>) {
                                        /*расчет и установка поля скидка йцу*/
                                        var discountPercent = per_discount; // процент скидки от общего кол-ва (брать из админки?)
                                        var skidka = Number((sum_sku*discountPercent)/100).toFixed(2);
                                        $('.discountPrice>p:nth-child(2n)').text(skidka+ ' руб.');
...
<div id="size-popup" style="display:none">
    <div class="size-popup--inner">
        <p class="size-title">Таблица размеров одежды и обуви</p>
        <p class="size-title--item">Мужская одежда</p>
        <div class="man">
            <div class="size-column">
                <img src="/upload/product-images/mans.png" alt="">
            </div>
            <div class="size-column table-template">
                    <table>
                        <thead>
                        <tr>
                            <th>Размер</th>
                    ...
                    <p>Снимать мерки рекомендуется в облегающей одежде и босиком. Вам понадобится
                        измерительная лента. При измерении нужно держать спину прямо, лента при этом
                        должна быть параллельна полу.</p>
                    <p>Обхват груди измеряется по самому выступающему месту груди спереди и по нижнему
                        краю лопаток сзади.</p>
                    <p>Обхват талии — по самой узкой части талии.</p>
            </div>
        </div>
        <p class="size-title--item">Женская одежда</p>
        <div class="woman">
            <div class="size-column">
                <img src="/upload/product-images/woman.png" alt="">
            </div>
            <div class="size-column table-template">
                <table>
                    <thead>
        ...
        <p class="size-title--item">Футболки, футболки «Престиж»</p>
        <div class="table-template-2">
            <table>
                <thead>
                <tr>
                    <th>Обозначение</th>
                    ...
                    <th>XXL</th>
                    <th>XXXL</th>
                    <th>XXXXL <span>(4XL)</span></th>
                    <th>XXXXXL <span>(5XL)</span></th>
                </tr>
                </thead>
                <tbody>
                <tr>
        ...
        <p class="size-title--item">Обувь</p>
        <div class="table-template-2 shoes">
            <table>
                <thead>
                <tr>
                    <th>Размер</th>
        ...
        <p class="size-popup-text-bottom">Размер обуви определяется по таблице — исходя из расстояния от пятки до кончика большого пальца. Чтобы измерить длину стопы точно, можно поставить ногу на лист бумаги и обвести её карандашом. Расстояние от наиболее выступающей части пятки до кончика большого пальца и будет длиной стопы.</p>
    </div>
</div>
<div id="discounts" style="display:none">
    <div  class="discounts--inner ">
        ...
        <ul class="discounts--content">
            <li>
                <p>до 30 000 руб.</p>
                <p>Розничная (цена сайта)</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>2% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>3% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>4% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>5% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>6% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>7% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>8% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>9% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>10% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>11% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>12% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>13% от розничной цены</p>
            </li>
            <li>
                <p>до 30 000 руб.</p>
                <p>14% от розничной цены</p>
            </li>
        </ul>
    </div>
</div>
<div class="color-hover" >
	<div class="color-hover--content">
		<div class="color-hover-inner">
			<img src="/upload/product-images/color-hover-img.png" alt="">
			<p>
				При выборе цвета вы переходите на страницу товара другого цвета
			</p>
		</div>
		<div class="color-hover-bottom">
		</div>
	</div>
</div>
<script type="text/javascript">
...
foreach($arBasketItems as $basketItem) {
    $strBasketItem = stristr($basketItem['PRODUCT_XML_ID'], '#', true);
    $arBasketUniqItems[] = $strBasketItem;
}
$arUniq = array_unique($arBasketUniqItems);
*/?>
<script>
    $( document ).ready(function() {
        ...
        $('.addtoCardbyAnkuse').click(function () {

            var data = $('.aff_form').serialize();
            console.log(data);
            $.ajax({
                url: arNextOptions['SITE_DIR'] + 'ajax/add2basketByAnkuse.php',
                data: data,
                type: "POST",
                success: function (response) {
                    console.log(response);
                    var pars = JSON.parse(response);
                    var inique_array = pars.INIQUE_ARRAY;
                    var curpriceBasket = 0;
                    inique_array.map((e)=>{
                        console.log(e);
                        var el = e.BASE_PRICE;
                        var quantity = e.QUANTITY;
                        var numberEl = Number(el);
                        console.log(typeof(quantity));
                        console.log(typeof(numberEl));
                        var resultSum = numberEl*quantity;
                        curpriceBasket += resultSum;
                        console.log(curpriceBasket);
                    });
                    ...
                    var size = Object.size(inique_array);
                    var countInBasketpars = size;
                    console.log(countInBasketpars);
                    $('.my-count-line-basket').html(countInBasketpars);
                    $('.my-prices-line-basket').html(curpriceBasket + ' руб.');
                    $('.js-basket-block').find('.svg-inline-basket').css('opacity', 1);
                    $('.my-count-line-basket').css('background-color', '#0088cc');
                    curpriceBasket = 0;
                }
            });
            return false;
